...
- **fix — a service whose key goes in the address asks for it like any other.** SABnzbd, Tautulli, Pi-hole v5 and Plex take their key in the query string and offer no header form, so the panel showed no sign-in block at all: the *API key* box vanished and the reader was left replacing `YOUR_KEY` in the address by hand. Plex was the worst of them — its key box asked for an `Accept` header, which is not a secret and is the same for every install, while the token that actually signs the request had to go in the address. All four are ordinary sign-ins now. The value goes to the credential file like any other, the stored address keeps its placeholder, and the server fills the parameter in per request — so the key stays out of `bookmarks-N.json` and out of every backup and export. A key already typed into an address by hand is left alone, because that is somebody's working widget.
