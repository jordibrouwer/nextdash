...
- **new** — **`BackgroundPattern`**: `dots`, `grid`, `lines`, `hatch` or `none`, with `auto` meaning the theme decides. Separate from `ShowBackgroundDots`, which stays the on/off — two controls that both switch the backdrop off would be one too many.
