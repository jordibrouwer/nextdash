...
- **fix** **What's new script cache bust** — `whats-new-modal.js?v=whats-new-v51` on dashboard and config templates so release-note updates load without a hard refresh.

#### Repo hygiene

- **fix** **`.gitignore` merge conflict** — resolved conflict markers; consolidated Go and project ignore rules; `node_modules/` ignored.
- **fix** **`node_modules` untracked** — Playwright removed from git history; `package.json` keeps Playwright as `devDependencies` only (`npm install` for local debug scripts).
