...
- **new** **Dashboard module split** — `static/js/dashboard/*.js` modules (data, render-core, setup, promos, ui-helpers, …); `dashboard.js` orchestrator; `docs/dashboard-refactor.md`; Playwright smoke tests for overlays, grid shortcuts (including `G` tap vs hold and `Shift+T` tag popover focus), command palette (live feedback, 10 groups, `:dark`, `:collections`, `:filter`, `:page`), search filters (`status:`/`tag:`/`page:` autocomplete, duplicate filter groups), recent modal tab caps, post-onboarding layout nudge, and health redirect detect (`tests/health-redirect.spec.js`).
