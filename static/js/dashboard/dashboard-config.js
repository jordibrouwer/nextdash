...
class DashboardConfig {
    ...
    render() {
        ...
        // view whose whole job is switching sections.
        if (this.reuseShell(container)) {
     ...
     * from another view — in which case the caller builds one.
     */
    reuseShell(container) {
     ...
     *
     * The entries live in static/data/overview-features.json: 42 of them, five
     * strings each, and every one of those strings also exists in the locale
     * files — so as code they were 48 KB of the config module that every visit
     * downloaded to draw a panel most of them never scroll. Fetched once, on the
     * first Overview render, and the panel simply does not appear until it is
     * there; it is context, not work.
     */
    overviewNewFeatures() {
        if (Array.isArray(this._overviewFeatures)) return this._overviewFeatures;
        void this.loadOverviewFeatures();
        return [];
    }

    /** Fetch the catalog once and repaint the panel when it lands. */
    loadOverviewFeatures() {
        if (this._overviewFeaturesPromise) return this._overviewFeaturesPromise;
        // Versioned by the app fingerprint rather than a content hash: the asset
        // hasher covers .js and .css only, and this is data — the same route the
        // what's-new files take.
        const version = document.querySelector('meta[name="nextdash-app-version"]')?.content || '';
        const url = version
            ? `/static/data/overview-features.json?v=${encodeURIComponent(version)}`
            : '/static/data/overview-features.json';
        this._overviewFeaturesPromise = fetch(url)
            .then((res) => (res.ok ? res.json() : null))
            .then((doc) => {
                const list = Array.isArray(doc?.features) ? doc.features : [];
                this._overviewFeatures = list;
                // The stream carries the features now, so a late catalogue
                // repaints the whole overview rather than one panel.
                if (list.length && this.isActiveView() && this.section === 'overview') {
                    this.repaintOverview();
                }
                return list;
            })
            .catch(() => {
                // An empty catalog renders no panel, which is what an install
                // with nothing to announce looks like anyway.
                this._overviewFeatures = [];
                return [];
            });
    ...
    renderNewsItem(item) {
        ...
        const label = {
            site: this.t('config.overviewNewsSourceSite', 'nextdash.cc'),
            release: this.t('config.overviewNewsSourceRelease', 'release'),
            feature: this.t('config.overviewNewsSourceFeature', 'feature'),
        }[item.source] || item.source;

        // A feature's words live in the locale files, so they are the reader's
        // words; a post's come from the feed as they were written.
        const title = item.titleKey ? this.t(item.titleKey, item.title) : item.title;
        let summary = item.summaryKey ? this.t(item.summaryKey, item.summary) : item.summary;
        if (item.source === 'release') {
            // The date is already in its own column, so repeating it as the
            // summary says nothing. The newest release has its lead loaded for
            // Help's version heading; older ones stand on their tag.
            const lead = this._latestRelease?.tag === item.title
                ? String(this._latestRelease.modalLead || '').replace(/<[^>]*>/g, '').trim()
                : '';
            summary = lead;
        }

        let action = '';
        if (item.source === 'site' && item.url) {
            action = `<a class="config-news-go" href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${esc(this.t('config.overviewNewsRead', 'Read on nextdash.cc ↗'))}</a>`;
        } else if (item.source === 'release') {
            action = `<button type="button" class="config-news-go" data-overview-action="whats-new">${esc(this.t('config.showWhatsNew', 'Show what’s new'))}</button>`;
        } else if (item.go) {
            action = `<button type="button" class="config-news-go" data-overview-go='${esc(JSON.stringify(item.go))}'>${esc(this.t(item.ctaKey, item.ctaFallback || ''))}</button>`;
        }
    ...
    /** Source filters, with what each would show. */
    renderNewsChips() {
        const esc = (v) => this.dash.escapeHtml(v);
        const stream = this._newsStream || [];
        const filter = this.newsFilter || 'all';
        const count = (source) => (source === 'all'
            ? stream.length
            : stream.filter((item) => item.source === source).length);

        const chip = (source, label) => `
            <button type="button" class="config-src-filter config-src-filter--${esc(source)}"
                    data-news-filter="${esc(source)}" aria-pressed="${filter === source ? 'true' : 'false'}">
                ${esc(label)} <b>${esc(String(count(source)))}</b>
            </button>`;

        return `
            <div class="config-news-chips" role="group" aria-label="${esc(this.t('config.overviewNewsFilterAria', 'Filter news by source'))}">
                ${chip('all', this.t('config.overviewNewsSourceAll', 'All'))}
                ${chip('site', this.t('config.overviewNewsSourceSite', 'nextdash.cc'))}
                ${chip('release', this.t('config.overviewNewsSourceReleases', 'Releases'))}
                ${chip('feature', this.t('config.overviewNewsSourceFeatures', 'Features'))}
            </div>`;
    }
    ...
    bindSectionNav(container) {
        // The rail survives a section switch now, so binding it again would add
        // a second handler to the same buttons on every render.
        const nav = container.querySelector('.config-nav-column') || container;
    ...
    renderStats() {
        ...
        void this.ensureStatsRenderers();
        const esc = (v) => this.dash.escapeHtml(v);
     ...
     * it, so returning nothing would leave the section without a foot — and
     * without the export and refresh buttons that live in it — until the whole
     * view rendered again.
     */
    renderStatsTimestampSafe() {
        if (typeof this.renderStatsTimestamp === 'function') {
            return this.renderStatsTimestamp();
        }
    ...
    ensureStatsRenderers() {
        ...
        return this._statsRenderersPromise;
    }
    ...
    repaintStatsBody() {
        ...
        if (foot) {
            foot.outerHTML = this.renderStatsTimestampSafe();
            // Replaced wholesale, so its buttons need binding again — before,
            // the foot only ever appeared in a full render, which bound them.
            const panel = document.getElementById('config-section-panel') || document;
            this.bindStats?.(panel);
        }
        const container = document.getElementById('dashboard-layout');
     ...
     * modal reads, so the two agree by construction; the plain product name is
     * what shows if that has not loaded, which is never wrong, only less
     * specific.
     */
    helpVersionHeading() {
        ...
        return `
            <div class="config-panel">
                ...
                <div class="help-about-mark">
                    <!-- 680px wide for a mark shown at most 340: the 2065px
                         original was 104 KB of a 340px panel. -->
                    <img src="/static/nextdash-wordmark.png" alt="nextDash"
                        width="680" height="185" loading="lazy" decoding="async">
                </div>
