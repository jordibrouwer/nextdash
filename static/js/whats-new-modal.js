...
(function () {
    ...
    window.__whatsNewOpen = function openWhatsNewModal(options) {
         ...
         *
         * The panel said "close" three times -- a full-width button, a hint
         * under it, and nothing in the header. `onHide` marks the release seen
         * whichever of them is used, so the button is the one that can go.
         */
        const wnHeader = document.querySelector('#app-modal .whats-new-modal .modal-header');
        if (wnHeader && !wnHeader.querySelector('.wn-modal-close')) {
