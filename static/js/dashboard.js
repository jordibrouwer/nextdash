...
class Dashboard {
    ...
    hasInlineEditUnsavedChanges() {
        ...
        const targetPageId = fields.pageSelect
            ...
            : Number(ctx.bookmarkRef.pageId || this.currentPageId);
        const originalPageId = Number(ctx.bookmarkRef.pageId || this.currentPageId);
    ...
    async requestPageNavigation(pageId) {
        ...
        if (targetPageId === Number(this.currentPageId)) {
            return true;
        }
        if (!(await this.confirmInlineEditBeforeNavigation())) {
            return false;
    ...
    renderPageNavigation() {
        ...
        if (this._pageNavKeyHandler) {
            container.removeEventListener('keydown', this._pageNavKeyHandler);
        }
        this._pageNavKeyHandler = (e) => {
        ...
        container.addEventListener('keydown', this._pageNavKeyHandler);
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (key >= '1' && key <= '9') {
                ...
                if (pageIndex < this.pages.length) {
                    ...
                    void this.requestPageNavigation(page.id).then((switched) => {
                        if (!switched) {
                            return;
                        }
                        this.markInlineTipUsed('page_switch');
    ...
    async showPageOverlay() {
        ...
        const onKey = (e) => {
            if (e.key === 'Escape' || e.key === ',') {
                ...
                if (idx < pages.length) {
                    e.preventDefault();
                    void this.requestPageNavigation(pages[idx].id).then((switched) => {
                        if (switched) {
                            close();
                        }
                    });
    ...
    applyBackground() {
        ...
        const showDots = this.settings.showBackgroundDots !== false;
        ...
        if (presetName) {
            ...
            customBackground = window.BookmarkUrlUtils?.safeCssImageUrl?.(this.settings.backgroundImageUrl) || '';
        }
    ...
    flushPendingDashboardSavesOnExit() {
        const hadReorder = Boolean(this.pendingReorderSnapshot) || Boolean(this._bookmarkOrderSaveInFlight);
    ...
    async saveBookmarkOrder(options = {}) {
        ...
        const priorSave = this._bookmarkOrderSaveInFlight;
        const saveTask = (async () => {
            if (priorSave) {
                try {
                    await priorSave;
    ...
    createCategoryElement(category, bookmarks) {
        ...
        const setCategoryCollapsed = (collapsed) => {
            ...
            this.collapsedCategories[collapsedKey] = collapsed;
            this.saveCollapsedStates();
        };
    ...
    getSmartCollections(bookmarks) {
        ...
        if (this.settings.showSmartRecentCollection !== false && pageAllowed(this.settings.smartRecentPageIds) && recentBookmarks.length > 0) {
            ...
            const effectiveLimit = Number.isFinite(configuredLimit) && configuredLimit > 0
                ? configuredLimit
                : null;
            const recentLabel = this.language?.t?.('dashboard.smartRecentCollection');
        ...
        if (this.settings.showSmartStaleCollection !== false && pageAllowed(this.settings.smartStalePageIds) && staleBookmarks.length > 0) {
            ...
            const effectiveLimit = Number.isFinite(configuredLimit) && configuredLimit > 0
                ? configuredLimit
                : null;
            const staleLabel = this.language?.t?.('dashboard.smartStaleCollection');
        ...
        if (this.settings.showSmartMostUsedCollection === true && pageAllowed(this.settings.smartMostUsedPageIds) && mostUsedBookmarks.length > 0) {
            ...
            const effectiveLimit = Number.isFinite(configuredLimit) && configuredLimit > 0
                ? configuredLimit
                : null;
            const mostUsedLabel = this.language?.t?.('dashboard.smartMostUsedCollection');
    ...
    toggleRecentBookmarksModal() {
        ...
        if (this.isModalOpen() && !this.isRecentBookmarksModalOpen()) return;
        ...
        if (!this._bookmarksReady) {
            this._pendingRecentModalRefresh = true;
            return;
        }
    ...
    _fillRecentBookmarksModal() {
        ...
        const shownWithUrls = this.getRecentBookmarksWithUrls(this.bookmarks, Dashboard.RECENT_MODAL_DISPLAY_LIMIT);
        const lastWithUrls = this.getRecentBookmarksWithUrls(this.bookmarks, Dashboard.OPEN_LAST_DEFAULT);
        ...
        const openToolbarHtml = openPlans.length > 0
            ? `<div class="recent-bookmarks-modal-toolbar" role="toolbar" aria-label="${this.escapeHtml(this.formatDashboardLabel('recentOpenToolbar', {}, 'Open recent bookmarks'))}">
                    ...
                    <p class="recent-bookmarks-open-hint">${this.escapeHtml(this.formatDashboardLabel('recentOpenCommandHint', { n: Dashboard.OPEN_LAST_DEFAULT }, `:open last ${Dashboard.OPEN_LAST_DEFAULT} in command mode`))}</p>
                </div>`
            : '';
    ...
    _setupRecentModalKeyboardNav(body) {
        ...
        document.addEventListener('keydown', this._recentModalKeyHandler, true);
    }

    _cleanupRecentModalKeyHandler() {
        if (!this._recentModalKeyHandler) {
            return;
        }
        document.removeEventListener('keydown', this._recentModalKeyHandler, true);
        this._recentModalKeyHandler = null;
    }
