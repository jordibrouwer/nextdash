...
| **Saving a link from a script** | [`integrations/`](integrations/) holds a shell script the rest build on (`nextdash-add <url> [title]`), two Raycast commands — one that takes a URL, one that reads the front tab of Safari, Chrome, Arc, Brave or Edge — and the four steps for an Apple Shortcut. On iOS the Shortcut is the route that works: Safari does not implement the web share target the installed app declares |
