...
**A left column, not a row of tiles** (**v1.6.1**). The Inbox shares its shell with Health: the filters — *All*, *Unread*, *Snoozed*, *With note* — are rows in a left column with their own counts, in place of a strip of tiles above the list that repeated the same numbers. **This week** sits above them as a plain readout. **Snoozed** and **With note** appear only when there is something in them, and drop out again once there isn't. The header — **Triage** and the **⋯** menu — stays visible while you scroll a long list, shrinking to a single bar as it sticks so the left column stays reachable too; on a narrow screen the left column becomes one scrolling line under the title, and **ℹ** joins the **⋯** menu. Rows can be **compact** or **comfortable**, the same setting Health reads.
