...
Since **v2026.08.04**, nextDash can compare your running release tag with the latest on GitHub once a day. When a newer version exists, Config → Overview shows a compact notice above Tips, the ★ button gets a dot, and a toast appears once per release while you are actively using the app. Press **Check for updates** on Overview to compare manually; since **v2026.08.08.1** the ★ modal header only reports the result, with a link to the release and **Dismiss**.
