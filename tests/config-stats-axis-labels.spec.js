...
test.describe('statistics: chart axis labels', () => {
    test('the activity chart names both axes', async ({ page }) => {
        ...
        const panel = panelByTitle(page, 'Opens over time');

        // y: what the bars count, plus a real top tick rather than an unlabelled scale.
        await expect(panel.locator('.config-chart-axis-title')).toHaveText(/opens/i);
    ...
    test('the x-axis label follows the selected range', async ({ page }) => {
        ...
        await panelByTitle(page, 'Opens over time').locator('[data-stats-range="90"]').click();
        await expect(panelByTitle(page, 'Opens over time').locator('.config-chart-axis-x')).toHaveText(/week/i);

        await panelByTitle(page, 'Opens over time').locator('[data-stats-range="365"]').click();
        await expect(panelByTitle(page, 'Opens over time').locator('.config-chart-axis-x')).toHaveText(/month/i);
    });
    ...
    test('the plot is tall enough to compare neighbouring bars', async ({ page }) => {
        await openStatsTab(page, 'activity');
        const svg = panelByTitle(page, 'Opens over time').locator('svg');
        ...
        const ticks = await panelByTitle(page, 'Opens over time')
    ...
    test('the x-axis carries dated ticks, not just its two ends', async ({ page }) => {
        await openStatsTab(page, 'activity');
        const ticks = panelByTitle(page, 'Opens over time').locator('.config-chart-tick');
    ...
    test('no tick escapes the plot or collides, at any range', async ({ page }) => {
        ...
        for (const range of ['7', '30', '90', '365']) {
            ...
            const after = panelByTitle(page, 'Opens over time');
