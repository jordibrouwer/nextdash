...
test.describe('a release flagged hideFromModal', () => {
    ...
    // reading; v1.7.0 released them from that hold and v1.7.1 leads now, so
    // a reader following the notes back finds every version in between.
    test('no shipped release is held back, and v1.7.1 leads the modal', async ({ page }) => {
        await loadDashboard(page);

        const index = await page.evaluate(async () =>
            (await fetch('/static/data/whats-new/index.json')).json());

        expect(index[0].tag).toBe('v1.7.1');
        ...
        // The modal leads with v1.7.1, and the two releases that used to be
        // held back are reachable in it.
        expect(await shownTags()).toContain('v1.7.1');
    ...
    test('the release constants name v1.7.1, the release the modal leads with', async ({ page }) => {
         ...
         * for v1.7.1, so it is also what index[0] names -- an install that
         * already read v1.7.0's notes is reopened once for this release.
         */
        expect(src).toContain("DASHBOARD_RELEASE = '2026.09-dashboard-release-v1.7.1'");
        // The data token moves regardless: the index changed, and a browser
        // holding its old copy would never learn v1.7.1 exists.
        expect(src).toContain("NEXTDASH_WHATS_NEW_DATA_VERSION = 'whats-new-v281'");
    });
});
