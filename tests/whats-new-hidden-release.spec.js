...
test.describe('a release flagged hideFromModal', () => {
    ...
    test('nothing is held back from the modal any more', async ({ page }) => {
        ...
        expect(index[0].tag).toBe('v1.4.7.1');
        ...
        expect(await shownTags()).toContain('v1.4.7.1');
    ...
    test('the release constants name v1.4.7.1, the release the modal leads with', async ({ page }) => {
         ...
         */
        expect(src).toContain("DASHBOARD_RELEASE = '2026.09-dashboard-release-v1.4.7.1'");
        // The data token moves regardless: the index changed, and a browser
        // holding its old copy would never learn v1.4.7.1 exists.
        expect(src).toContain("NEXTDASH_WHATS_NEW_DATA_VERSION = 'whats-new-v275'");
    });
});
