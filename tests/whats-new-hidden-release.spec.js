...
test.describe('a release flagged hideFromModal', () => {
    ...
    test('v1.4.2.2 leads the index while the modal still leads with v1.4.2', async ({ page }) => {
        await loadDashboard(page);
        ...
        expect(index[0].tag).toBe('v1.4.2.2');
        expect(index[0].hideFromModal).toBe(true);
        // Two hidden releases in a row now, which is the arrangement that can
        // quietly go wrong: the modal has to step over both rather than the
        // first one only.
        expect(index[1].tag).toBe('v1.4.2.1');
        expect(index[1].hideFromModal).toBe(true);
        expect(index[2].tag).toBe('v1.4.2');
        ...
        // v1.4.2 leads the modal even though v1.4.2.1 is newer, and v1.2.1 is
    ...
    test('the release constants name v1.4.2, the release the modal leads with', async ({ page }) => {
        ...
        // The release token stays on v1.4.2 on purpose: v1.4.2.1 and v1.4.2.2
        ...
        // learns v1.4.2.2 exists.
        expect(src).toContain("NEXTDASH_WHATS_NEW_DATA_VERSION = 'whats-new-v263'");
    });
});
