...
test.describe('a release flagged hideFromModal', () => {
    ...
    test('v2026.09.06.1 release constants are bumped', async ({ page }) => {
        const stub = await page.request.get('/static/js/whats-new-stub.js');
        const src = await stub.text();
        expect(src).toContain("DASHBOARD_RELEASE = '2026.07-dashboard-release-v174'");
        expect(src).toContain("NEXTDASH_WHATS_NEW_DATA_VERSION = 'whats-new-v233'");
    });
});
