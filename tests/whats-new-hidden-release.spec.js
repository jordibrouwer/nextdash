...
test.describe('a release flagged hideFromModal', () => {
    ...
    test('v1.3.1 leads the index and the modal, with v1.2.1 still hidden behind it', async ({ page }) => {
        await loadDashboard(page);
        ...
        expect(index[0].tag).toBe('v1.3.1');
        expect(index[0].hideFromModal).toBeUndefined();
        expect(index[1].tag).toBe('v1.3.0');
        ...
        expect(index[2].tag).toBe('v1.2.1');
        ...
        // v1.3.1 leads; v1.2.1 is skipped wherever the reader scrolls, which is
        // the whole of what hideFromModal promises.
        expect(await shownTags()).toContain('v1.3.1');
    ...
    test('the release constants name v1.3.1, the release the modal leads with', async ({ page }) => {
        const stub = await page.request.get('/static/js/whats-new-stub.js');
        const src = await stub.text();
        expect(src).toContain("DASHBOARD_RELEASE = '2026.08-dashboard-release-v1.3.1'");
        expect(src).toContain("NEXTDASH_WHATS_NEW_DATA_VERSION = 'whats-new-v247'");
    });
});
