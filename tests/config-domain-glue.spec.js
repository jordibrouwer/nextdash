...
async function waitForConfigReady(page) {
    ...
    await page.waitForFunction(() => typeof window.configManager?.pagesController !== 'undefined');
    await page.waitForFunction(() => typeof window.configManager?.getVisiblePages === 'function');
    await page.waitForSelector('.general-layout', { timeout: 20_000 });
    await page.evaluate(() => window.configManager.ui.switchToTab('general'));
}

test.describe('config domain glue (phase 4 controllers)', () => {
