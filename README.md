...
- **Typing filters; `Enter` opens** (**v1.2.0**) — a bookmark shortcut used to fire the moment your query matched it exactly, so an ordinary word was swallowed mid-word and which of your words survived depended on which *other* bookmarks you owned. Typing now narrows the list and `Enter` opens the top result, with the exact shortcut match leading it. Letters reach the grid only while a row is selected, so `g`, `j` and `k` no longer eat a first letter; the query line shows the key that started it (`>`, `:`, `?`) with an **×** to clear
- **…and it is a choice** — **Config → Behavior → Search → Typing a bookmark shortcut** offers three: press `Enter` (the default), open after a short pause once you stop typing, or open the moment the shortcut matches — the pre-v1.2.0 behaviour, kept for anyone who wants it back. The ℹ beside it spells out what each one costs
...
- **[`integrations/`](integrations/)** — a shell script, two Raycast commands and Apple Shortcuts recipes, all on top of the same one-line `/add` route
