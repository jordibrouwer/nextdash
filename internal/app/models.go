...
type Settings struct {
	...
	SurfaceFollowMigrated           bool   `json:"surfaceFollowMigrated,omitempty"`           // one-time: depth, glow and effects moved to follow-the-theme
	...
	/*
	 * ThemeEffects is how loudly a theme's archetype gets to speak.
	 *
	 * Everything an archetype draws -- the sheen, the glow, the grain, the
	 * corner radius -- multiplies by one factor, so off is genuinely off and
	 * there is no second place an effect can hide. Not the same question as
	 * depth: depth says how much of the theme is drawn behind the content,
	 * effects says how far the archetype goes. A reader who wants the depth
	 * and not the shine sets depth high and this to off.
	 *
	 * follow | off | held | full. Empty means follow.
	 */
	ThemeEffects string `json:"themeEffects,omitempty"`

	/*
	 * ThemeSurfacePrefs is what the reader changed, per theme.
	 *
	 * With the three settings above on "follow", a theme brings its own
	 * surfaces when it is picked. Changing one then belongs to that theme
	 * rather than to the install, so switching away and back finds it again
	 * and switching to a different theme finds that theme's own answer. Reset
	 * theme is a delete from this map; see theme_surfaces.go.
	 */
	ThemeSurfacePrefs map[string]ThemeSurfacePref `json:"themeSurfacePrefs,omitempty"`

	/*
	 * ThemeSurfacesForceAll says which of the two places the three answers
	 * above live in.
	 *
	 * Off -- the default -- a change belongs to the theme on screen and each
	 * theme keeps its own. On, the three settings hold for every theme and a
	 * theme brings nothing of its own, which is the arrangement every install
	 * had before this release and the one somebody who picked flat on purpose
	 * is asking for.
	 */
	ThemeSurfacesForceAll bool `json:"themeSurfacesForceAll,omitempty"`
...
type ThemeColors struct {
	 ...
	 * them per theme is how 218 of the 242 entries ended up saying nothing at
	 * all: every one of them was a separate decision nobody had a reason to
	 * make. An archetype is one word that answers all nine at once -- what
	 * kind of thing a surface is, as opposed to what colour it is.
	 *
	 * It never wins. An archetype fills in what a theme leaves blank, and any
	 * field the theme states for itself beats it (see resolveArchetype). So
	 * the 24 entries that already carry character keep exactly what they have.
	 */

	// Character names the archetype, from themeArchetypes. Empty means the
	// theme says nothing and the derivations behave exactly as before.
	Character string `json:"character,omitempty"`

	// GrainAngle and GrainScale draw the repeating light a brushed or woven
	// surface catches: an angle in degrees, and a strength of 0 to 1. Zero
	// scale draws nothing, which is every theme that does not ask.
	GrainAngle float64 `json:"grainAngle,omitempty"`
	GrainScale float64 `json:"grainScale,omitempty"`

	/*
	 * The surfaces this theme was drawn for.
	 *
	 * Depth and Glow and Effects are settings the reader owns, but each theme
	 * has an answer that suits it: a glass palette wants a different depth
	 * than the same archetype on black. With the setting on "follow", these
	 * are what a theme brings with it when it is picked; the reader can still
	 * change them, per theme, and Reset theme comes back here.
	 *
	 * Empty means work it out -- themeIdealDepth and friends derive one from
	 * the palette and the archetype, so a theme that says nothing is not flat
	 * by omission.
	 */
...
func (fs *FileStore) GetSettings() Settings {
	...
	if err := json.Unmarshal(data, &rawSettings); err == nil {
		...
		default:
			settings.ThemeDepth = surfaceFollow
		}
		switch settings.GlowStrength {
		case "off", "soft", "full", surfaceFollow:
		default:
			settings.GlowStrength = surfaceFollow
		}
		switch settings.ThemeEffects {
		case "off", "held", "full", surfaceFollow:
		default:
			settings.ThemeEffects = surfaceFollow
		}
		// Written by the client, read on every render: only words the
		// stylesheet knows may reach an attribute. Themes are not checked here
		// -- the store knows which exist, this does not -- so the map is
		// cleaned of values, and resolveSurfaces simply finds nothing for a
		// theme that has gone.
		...
		/*
		 * And then the question stops being one answer per install.
		 *
		 * Depth, glow and effects move to "follow", where the theme answers.
		 * This runs after the two passes above rather than instead of them:
		 * their markers still have to be written, or an install that later
		 * forces a depth would be dragged through them a second time.
		 *
		 * Everyone moves, including an install with a stored answer. Settings
		 * are written as one whole struct, so a depth is present in the file
		 * on nearly every install whether or not anybody chose it -- "key
		 * present" is not "choice made", and guessing which is which is the
		 * one thing this must not do. So: everybody follows, the What's new
		 * modal says so, and one setting puts it back. The marker makes it
		 * once, so a value forced afterwards is kept.
		 */
		if !settings.SurfaceFollowMigrated {
			settings.ThemeDepth = surfaceFollow
			settings.GlowStrength = surfaceFollow
			settings.ThemeEffects = surfaceFollow
			settings.SurfaceFollowMigrated = true
		}
		/*
...
func getDefaultBuiltInThemes() map[string]ThemeColors {
	return map[string]ThemeColors{
		 ...
		 */
		"aurora-glass-dark":   {Name: "Aurora Glass [dark]", TextPrimary: "#E9EDFC", TextSecondary: "#A9B4DA", TextTertiary: "#6E7BAA", BackgroundPrimary: "#0B1020", BackgroundSecondary: "#141B33", BackgroundDots: "#1E2748", BackgroundModal: "rgba(11, 16, 32, 0.88)", BorderPrimary: "#27314F", BorderSecondary: "#1B2440", AccentPrimary: "#7DA2FF", AccentSuccess: "#46D399", AccentWarning: "#F2B441", AccentError: "#FB7185", SurfaceAlpha: 0.58, SurfaceBlur: 20, SurfaceGlow: 1, RadiusScale: 1.4, LabelSpacing: "0.01em", LabelWeight: 600, Character: "glass"},
		"aurora-glass-light":  {Name: "Aurora Glass [light]", TextPrimary: "#1B2340", TextSecondary: "#454F73", TextTertiary: "#6B759B", BackgroundPrimary: "#F4F6FE", BackgroundSecondary: "#E9EDFB", BackgroundDots: "#D8DFF6", BackgroundModal: "rgba(244, 246, 254, 0.9)", BorderPrimary: "#CBD5F0", BorderSecondary: "#DFE5F7", AccentPrimary: "#3D63D8", AccentSuccess: "#0F8A5F", AccentWarning: "#9A6B00", AccentError: "#C2405A", SurfaceAlpha: 0.62, SurfaceBlur: 18, SurfaceGlow: 0.6, RadiusScale: 1.4, LabelSpacing: "0.01em", LabelWeight: 600, Character: "glass"},
		"nocturne-ink-dark":   {Name: "Nocturne Ink [dark]", TextPrimary: "#F5F3EF", TextSecondary: "#C6C2B9", TextTertiary: "#8E8A82", BackgroundPrimary: "#08090B", BackgroundSecondary: "#101216", BackgroundDots: "#1A1D22", BackgroundModal: "rgba(8, 9, 11, 0.92)", BorderPrimary: "#23262C", BorderSecondary: "#191C21", AccentPrimary: "#E4B363", AccentSuccess: "#7FB069", AccentWarning: "#E0A458", AccentError: "#D45D5D", SurfaceGlow: -1, RadiusScale: 0.5, LabelSpacing: "0.005em", LabelWeight: 500, Character: "ink"},
		"nocturne-ink-light":  {Name: "Nocturne Ink [light]", TextPrimary: "#17181B", TextSecondary: "#4A4B50", TextTertiary: "#75767C", BackgroundPrimary: "#FCFBF9", BackgroundSecondary: "#F2F1EC", BackgroundDots: "#E3E2DB", BackgroundModal: "rgba(252, 251, 249, 0.93)", BorderPrimary: "#DEDCD4", BorderSecondary: "#EAE8E1", AccentPrimary: "#9A6B1F", AccentSuccess: "#4A7A38", AccentWarning: "#9A6B1F", AccentError: "#A63D3D", SurfaceGlow: -1, RadiusScale: 0.5, LabelSpacing: "0.005em", LabelWeight: 500, Character: "ink"},
		"retro-crt-mk2-dark":  {Name: "Retro CRT Mk II [dark]", TextPrimary: "#D9FFDD", TextSecondary: "#86DF95", TextTertiary: "#4E9A5C", BackgroundPrimary: "#050705", BackgroundSecondary: "#0C140C", BackgroundDots: "#132013", BackgroundModal: "rgba(5, 7, 5, 0.92)", BorderPrimary: "#17361B", BorderSecondary: "#0F2412", AccentPrimary: "#39FF6A", AccentSuccess: "#39FF6A", AccentWarning: "#E8FF5A", AccentError: "#FF5A6E", SurfaceAlpha: 0.9, SurfaceGlow: 1, RadiusScale: 0.05, LabelTransform: "uppercase", LabelSpacing: "0.14em", LabelWeight: 700, Character: "terminal"},
		"retro-crt-mk2-light": {Name: "Retro CRT Mk II [light]", TextPrimary: "#082410", TextSecondary: "#1F5B2E", TextTertiary: "#3D7A4C", BackgroundPrimary: "#F3FAF3", BackgroundSecondary: "#E6F3E7", BackgroundDots: "#D2E8D5", BackgroundModal: "rgba(243, 250, 243, 0.93)", BorderPrimary: "#BEDCC2", BorderSecondary: "#D8ECDA", AccentPrimary: "#1B7A38", AccentSuccess: "#1B7A38", AccentWarning: "#7A6B00", AccentError: "#B03040", SurfaceAlpha: 0.9, SurfaceGlow: 0.5, RadiusScale: 0.05, LabelTransform: "uppercase", LabelSpacing: "0.14em", LabelWeight: 700, Character: "terminal"},
		"porcelain-dark":      {Name: "Porcelain [dark]", TextPrimary: "#F2EFE8", TextSecondary: "#C2BDB2", TextTertiary: "#8C877C", BackgroundPrimary: "#16150F", BackgroundSecondary: "#1F1E17", BackgroundDots: "#2A2820", BackgroundModal: "rgba(22, 21, 15, 0.92)", BorderPrimary: "#332F26", BorderSecondary: "#25231C", AccentPrimary: "#7FC0AC", AccentSuccess: "#7FC0AC", AccentWarning: "#D6A75A", AccentError: "#D97070", SurfaceGlow: -1, RadiusScale: 0.8, LabelSpacing: "0.01em", LabelWeight: 600, Character: "enamel"},
		"porcelain-light":     {Name: "Porcelain [light]", TextPrimary: "#22201C", TextSecondary: "#56524A", TextTertiary: "#8B857A", BackgroundPrimary: "#FBFAF7", BackgroundSecondary: "#F1EFE9", BackgroundDots: "#E4E1D8", BackgroundModal: "rgba(251, 250, 247, 0.93)", BorderPrimary: "#E2DED4", BorderSecondary: "#EDEAE2", AccentPrimary: "#2F6F5E", AccentSuccess: "#2F6F5E", AccentWarning: "#9A6B1F", AccentError: "#A33A3A", SurfaceGlow: -1, RadiusScale: 0.8, LabelSpacing: "0.01em", LabelWeight: 600, Character: "enamel"},
...
func fillThemeCharacter(current, defaults ThemeColors) ThemeColors {
	...
	// The archetype and what it draws. Every built-in was written to disk on
	// first run, so without this an install that has been going a week would
	// keep 242 stored palettes with no character at all and see nothing of
	// this release -- which is exactly what the field above it documents.
	if strings.TrimSpace(current.Character) == "" {
		current.Character = defaults.Character
	}
	if current.GrainAngle == 0 {
		current.GrainAngle = defaults.GrainAngle
	}
	if current.GrainScale == 0 {
		current.GrainScale = defaults.GrainScale
	}
	if strings.TrimSpace(current.Depth) == "" {
		current.Depth = defaults.Depth
	}
	if strings.TrimSpace(current.Glow) == "" {
		current.Glow = defaults.Glow
	}
	if strings.TrimSpace(current.Effects) == "" {
		current.Effects = defaults.Effects
	}
	return current
}
