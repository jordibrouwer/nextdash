...
func TestLazyLoadedAssetsExist(t *testing.T) {
	for _, rel := range lazyLoadedAssets {
		if _, err := os.Stat(repoFile(t, "static", filepath.FromSlash(rel))); err != nil {
...
func TestPreloadedFontURLsMatchStylesheet(t *testing.T) {
	tpl, err := os.ReadFile(repoFile(t, "templates", "dashboard.html"))
	if err != nil {
		t.Fatalf("read template: %v", err)
	}
	css, err := os.ReadFile(repoFile(t, "static", "css", "fonts.css"))
	if err != nil {
